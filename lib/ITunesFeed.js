...
class ITunesFeed {
    ...
    async _getFeed(cfg, feedType, genre, options) {
        ...
        try {
            let { data } = await this._axios.get(url, opts.axiosOptions);
            
            if(!Array.isArray(data.feed.results)) data.feed.results = [];
            return new Feed(data);
        } catch(e) {
            e.url = url;
            if(e.response && e.response.status === 404) e.message = 'iTunes Feed not found. Please check your parameters.';
